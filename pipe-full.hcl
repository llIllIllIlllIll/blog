...
####################################################################
#    Name:Yang Xiao ID:517370910062			           #
#    Discription:					           #
#    Since in ncopy addq function appears a lot of times ,iaddq is #
#    a must for optimization.                                      #
#    To make sure my hcl works, let't think about it. iaddq needs  #
#    nothing from memory, so load/use hazard has nothing to do with#
#    my implementation. Besides, I have nothing to do with ret or  #
#    jxx. So this implementation should be simple. What's more, the#
#    existing forwarding part can be applied to iaddq without any  #
#    change... It's like a dream. So basically I just added logic  #
#    about when to use rB, valC or what is dstE... It's really ez. #	
#    								   #
#    2. Also add a forwarding aiming at improving load/use hazard  #	      
#    to avoid one bubble in such case. 				   #						   
####################################################################
...
word f_pc = [
	# When E andq D jle and E result >0
 	E_icode == IOPQ && E_ifun == 2 && D_icode == IJXX && D_ifun ==1 && E_valA>0 && E_valB >0: D_valP;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && !(M_ifun == 1 && E_icode == IIADDQ): M_valA;
...
#Thank you ~ I don't think I need to do anything here
word d_valB = [
...
word aluA = [
	M_icode in { IMRMOVQ,IPOPQ} && E_icode == IOPQ && M_dstM == E_srcA: m_valM;
	E_icode in { IRRMOVQ, IOPQ } : E_valA;
	#E_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ} : E_valC;
	#E_icode == IIADDQ && M_icode == IOPQ && M_valE <=0 : 0;
	#( E_icode == IIADDQ && W_icode == IOPQ && W_valE <=0):0;
	E_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ} : E_valC;
...
word aluB = [
	M_icode in { IMRMOVQ,IPOPQ} && E_icode == IOPQ && M_dstM == E_srcB: m_valM;
...
bool D_bubble =
	...
	#(E_icode == IOPQ && E_ifun == 2 && D_icode == IJXX && D_icode == 1 && E_valA >0 && E_valB >0) ||
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd &&!(E_ifun ==1 && M_icode == IOPQ && M_ifun == 2 && M_valE >0 && D_icode == IIADDQ)) ||
	# Stalling at fetch while ret passes through pipeline
